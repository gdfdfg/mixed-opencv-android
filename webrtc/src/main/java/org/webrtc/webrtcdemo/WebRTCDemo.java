...
public class WebRTCDemo extends Activity implements MenuStateProvider {
  ...
  private NativeWebRtcContextRegistry contextRegistry = null;//注册本地服务
  private MediaEngine mediaEngine = null;          //得到媒体引擎
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    contextRegistry = new NativeWebRtcContextRegistry();//初始化webrtcdemo-jni
    contextRegistry.register(this);//在jni层进行注册

    // Load all settings dictated in xml.
    mediaEngine = new MediaEngine(this);//初始化MediaEngine
    mediaEngine.setRemoteIp(getResources().getString(R.string.loopbackIp));//初始化远程Ip
    mediaEngine.setTrace(getResources().getBoolean(
        R.bool.trace_enabled_default));//初始化是否开启

    mediaEngine.setAudio(getResources().getBoolean(
        R.bool.audio_enabled_default));//初始化是否开启audio
    mediaEngine.setAudioCodec(mediaEngine.getIsacIndex());//初始化AudioCode
    mediaEngine.setAudioRxPort(getResources().getInteger(
        R.integer.aRxPortDefault));//初始化AudioRxPort
    mediaEngine.setAudioTxPort(getResources().getInteger(
        R.integer.aTxPortDefault));//初始化AudioTxPort
    mediaEngine.setSpeaker(getResources().getBoolean(
        R.bool.speaker_enabled_default));//是否有Speaker
    mediaEngine.setDebuging(getResources().getBoolean(
        R.bool.apm_debug_enabled_default));//是否处于测试阶段

    mediaEngine.setReceiveVideo(getResources().getBoolean(
        R.bool.video_receive_enabled_default));//是否接受Video
    mediaEngine.setSendVideo(getResources().getBoolean(
        R.bool.video_send_enabled_default));//是否发送Video
    mediaEngine.setVideoCodec(getResources().getInteger(
        R.integer.video_codec_default));//设置VideoCodec
    ...
    mediaEngine.setVideoTxPort(getResources().getInteger(
        R.integer.vTxPortDefault));//设置VideoPort
    mediaEngine.setVideoRxPort(getResources().getInteger(
        R.integer.vRxPortDefault));//设置ViewPxPort
    mediaEngine.setNack(getResources().getBoolean(R.bool.nack_enabled_default));//设置是否否定回答
    mediaEngine.setViewSelection(getResources().getInteger(
        R.integer.defaultView));//设置查看的view
