...
public class MediaEngine implements VideoDecodeEncodeObserver {
  ...
  public void startVoE() {
    ...
    check(voe.startListen(audioChannel) == 0, "Failed StartListen");//native ����
    check(voe.startPlayout(audioChannel) == 0, "VoE start playout failed");//����native����
    check(voe.startSend(audioChannel) == 0, "VoE start send failed");//��ʼ����voice
    voeRunning = true;
  }
  ...
  public void startViE() {
    ...
    if (receiveVideo) {
      if (viewSelection ==
        ...
        svRemote = ViERenderer.CreateRenderer(context, true);//�������Ͷ���SurfaceView
      } else if (viewSelection ==
          context.getResources().getInteger(R.integer.surfaceView)) {
        svRemote = ViERenderer.CreateRenderer(context, false);//�������Ͷ���SurfaceView
      } else {
        externalCodec = new MediaCodecVideoDecoder(context);//ʹ���Լ���medai������
      ...
      if (externalCodec != null) {
        check(vie.registerExternalReceiveCodec(videoChannel,
            ...
            "Failed to register external decoder");//���Լ��ļ��������뵽VideoEngine��
      } else {
    ...
    if (sendVideo) {
      startCamera();//��ʼ��׽�Լ�����ͷ����
      check(vie.startSend(videoChannel) == 0, "Failed StartSend");//��鷢��״̬
    }
    vieRunning = true;
  }
  ...
  private void startCamera() {
    CameraDesc cameraInfo = vie.getCaptureDevice(getCameraId(getCameraIndex()));//�����ֻ���ǰ������ͷ���Ǻ�������ͷ���Ӳ�õ����沶׽�豸��
    currentCameraHandle = vie.allocateCaptureDevice(cameraInfo);//�õ���ǰ��׽���豸��
    cameraInfo.dispose();//�����ر�cameraInfo
    check(vie.connectCaptureDevice(currentCameraHandle, videoChannel) == 0,
        "Failed to connect capture device");//���Ӳ�׽�豸������������ǵ�����ͷ
    // Camera and preview surface.
    svLocal = new SurfaceView(context);//���������Լ�����������
    VideoCaptureAndroid.setLocalPreview(svLocal.getHolder());//�����Լ����豸����
    check(vie.startCapture(currentCameraHandle) == 0, "Failed StartCapture");//���Ϳ�ʼ��׽��Ϣ
    compensateRotation();
  }
